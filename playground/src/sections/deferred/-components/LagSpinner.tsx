...
function* SpeedRing({
  ...
  const tails = yield* useMemo(
    (all, from, turn, bandEdges) => {
  ...
  return <Clock tails={tails} size={DIAL_SIZE} style={{ color, ...ringStyle }} />;
}
 ...
 */
function* LagDial({ stamps, start }: { stamps: readonly number[]; start: number }) {
  return (
    <div>
      <span
...
function* LagChart({ stamps, now }: { stamps: readonly number[]; now: number }) {
  ...
  yield* useEffect(() => {
    ...
    const observer = new ResizeObserver(([entry]) => {
      ...
      void setSize({ width: rect.width, height: rect.height });
    });
  ...
  const bars = yield* useMemo(
    (all, end) => {
      const from = end - CHART_WINDOW_MS;
      return all
        .slice(0, -1)
        .map((stamp, index) => ({ stamp, gap: all[index + 1]! - stamp }))
...
export default function* LagSpinner() {
  ...
  const [stamps, setStamps] = yield* useState<number[]>([]);

  yield* useEffect(() => {
    let handle = requestAnimationFrame(function animate() {
      void setStamps((current) => {
        const now = Date.now();
